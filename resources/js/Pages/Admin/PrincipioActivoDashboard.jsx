...
export default function PrincipioActivoDashboard({ principiosActivos }) {
    ...
    const filteredPrincipiosActivos = principiosActivos.filter(principio =>
        principio.nombre.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (principio.descripcion && principio.descripcion.toLowerCase().includes(searchTerm.toLowerCase()))
    );
                ...
                <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            ...
                            <p className="text-gray-600">Administra los principios activos de tus productos</p>
                        </div>
                    ...
                    <div className="mb-6 backdrop-blur-2xl bg-white/60 border-2 border-emerald-200/40 rounded-2xl shadow-xl p-6">
                        <div className="relative">
                            ...
                            <input
                                type="text"
                                placeholder="Buscar principios activos por nombre o descripción..."
                                value={searchTerm}
                        ...
                        {searchTerm && (
                            <p className="mt-3 text-sm text-emerald-600 font-medium">
                                Mostrando {filteredPrincipiosActivos.length} de {principiosActivos.length} principios activos
                            </p>
                        )}
                    </div>

                    {/* Cards Grid */}
                    {filteredPrincipiosActivos.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {filteredPrincipiosActivos.map((principioActivo) => (
                                <div key={principioActivo.id} className="group backdrop-blur-2xl bg-white/60 border-2 border-emerald-200/40 rounded-2xl shadow-xl hover:shadow-2xl hover:border-emerald-300/60 transition-all duration-300 overflow-hidden">
                                    <div className="p-6">
                        ...
                        <div className="backdrop-blur-2xl bg-white/60 border-2 border-emerald-200/40 rounded-2xl shadow-xl p-12">
                            <div className="text-center">
                                ...
                                <h3 className="text-2xl font-bold text-gray-800 mb-2">
                                    {searchTerm ? 'No se encontraron principios activos' : 'No hay principios activos'}
                                </h3>
                            ...
                            <div className="sm:flex sm:items-start">
                                ...
                                <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                                    ...
                                    <p className="text-sm text-gray-600">
                                        ¿Estás seguro que deseas eliminar el principio activo <span className="font-bold text-gray-800">"{principioActivoToDelete?.nombre}"</span>?
                                    </p>
